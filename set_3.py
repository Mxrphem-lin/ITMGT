...
def tic_tac_toe(board):
    ...
    for col in range(n):
        column_current = [board[r][col] for r in range(n)]
        if len(set(column_current)) == 1 and column_current[0] != "":
            return column_current[0]
        
    left_right = [board[i][i] for i in range(n)]
    if len(set(left_right)) == 1 and left_right[0] != "":
        return left_right[0]
    
    right_left = [board[i][n - 1 - i] for i in range(n)]
    if len(set(right_left)) == 1 and right_left[0] != "":
        return right_left[0]
...
def eta(first_stop, second_stop, route_map):
    ...
    while current != second_stop:
        for (start, end), value in route_map.items():
            if start == current:
                time += value["travel_time_mins"]
                current = end
                break
